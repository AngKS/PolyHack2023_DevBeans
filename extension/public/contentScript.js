...
const revealTweet = (e, height) => {
  if (height < 250) {
    ...
    e.target.parentElement.parentElement.previousElementSibling.style.filter = "blur(0px)";
  } else {
    e.target.parentElement.parentElement.setAttribute("hidden", "");
    e.target.parentElement.parentElement.previousElementSibling.style.filter = "blur(0px)";
  }
...
const getTopicsSentiments = async (cleanedText) => {
  ...
  chrome.storage.local.get(null, function (result) {
    ...
    const postData = {
      "tweet_sentiment": filteredLabels.length === 0 ? { labels: [] } : { labels: filteredLabels },
